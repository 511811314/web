...
function pager_query($sql,&$nav_html,$sdept,$page_current=1, $page_size=3){
	...
	else
	{
		...
		$length = strpos($sql,'=')-strpos($sql,'from')+1; 

		$sql_amount = "select count(*) as amount " . substr($sql, strpos($sql,'from'), $length).$sdept;

		$sqla = "select * ". substr($sql, strpos($sql,'from'), $length).$sdept;
